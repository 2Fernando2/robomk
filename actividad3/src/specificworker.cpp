...
void SpecificWorker::initialize() {
  ...
  if (this->startup_check_flag) {
    ...
    room_draw = viewer_room->scene.addRect(nominal_rooms[0].rect(),
                                           QPen(Qt::black, 30));
...
void SpecificWorker::compute() {
  ...
  const auto &[filtered_points, doors_] =
      door_detector.filter_points(points, &viewer->scene);
  doors = doors_;
  // draw_lidar(points, &viewer->scene);

  // compute corners
  const auto &[corners, lines] =
      room_detector.compute_corners(filtered_points, &viewer->scene);
  ...
  if (center_opt.has_value())
    ...
    std::cerr << "No center room point found" << std::endl;
    return;
  }
  ...
  if (not match.empty()) {
    const auto max_error_iter =
        std::ranges::max_element(match, [](const auto &a, const auto &b) {
  ...
  RetVal ret_val =
      state_machine(filtered_points, match, corners, max_match_error);
  ...
  //" Center: " << center_opt.value().norm(); // << " Angle_to_center: " <<
  // angle_;
  ...
  const double angle = qRadiansToDegrees(
...
SpecificWorker::state_machine(const RoboCompLidar3D::TPoints &points,
                              const Match &match, const Corners &corners,
...
SpecificWorker::do_work(const Eigen::Vector2d target) {
  // rotation
...
void SpecificWorker::draw_target(const Eigen::Vector2d &target,
                                 QGraphicsScene *scene, bool last_iteratior) {
  static std::vector<QGraphicsItem *>
      items; // store items so they can be shown between iterations
  ...
  auto item =
      scene->addRect(-100, -100, 200, 200, QColor(QColorConstants::Svg::silver),
                     QBrush(QColorConstants::Svg::silver));
...
SpecificWorker::RetVal SpecificWorker::turn(const Corners &corners) {
  if (const auto &[success, giro] = image_processor.check_colour_patch_in_image(
          camera360rgb_proxy, colors[room_index], label_img);
      success) {
    // localised = ... localise(..)
    // if (localised) return {..}
    // else return {..}
...
SpecificWorker::RetVal SpecificWorker::goto_door(const RoboCompLidar3D::TPoints &points,
  ...
  Eigen::Vector2d robot_position(robot_pose.translation().x(),
                                 robot_pose.translation().y());
  const auto target = doors[0].center_before(Eigen::Vector2d(0.f,0.f), 650.f);
...
SpecificWorker::RetVal SpecificWorker::orient_to_door(const float &max_match_error) {
  ...
  if (elapsed > 3000) {
    first_time = true;
    if (max_match_error > params.RELOCAL_DONE_MATCH_MAX_ERROR or
        not door_center.any()) {
      localised = false;
      return {STATE::GOTO_ROOM_CENTER, 0.f, 0.f};
    }
  ...
  return {STATE::ORIENT_TO_DOOR, adv_vel, rot_vel};
}
...
SpecificWorker::RetVal SpecificWorker::cross_door(const RoboCompLidar3D::TPoints &points) {
  ...
  if (elapsed > 2000) {
    ...
    // this->dimensions); auto [rr, re] =
    // viewer_room->add_robot(params.ROBOT_WIDTH, params.ROBOT_LENGTH, 0, 100,
    // QColor("Blue")); robot_room_draw = rr;
    // viewer_room->updateSceneRect(robot_room_draw);

    // draw room in viewer_room
    room_draw = viewer_room->scene.addRect(nominal_rooms[room_index].rect(),
                                           QPen(Qt::black, 30));
    // show();

    return {STATE::GOTO_ROOM_CENTER, 0.f, 0.f};
  }
  return {STATE::CROSS_DOOR, 650.f, 0.f};
}

SpecificWorker::RetVal SpecificWorker::update_pose(const Corners &corners,
                                                   const Match &match) {
  return {};
}
...
void SpecificWorker::update_robot_pose(const Corners &corners,
                                       const Match &match) {
...
SpecificWorker::closest_lidar_index_to_given_angle(const auto &points,
  ...
  auto res =
      std::ranges::find_if(points, [angle](auto &a) { return a.phi > angle; });
...
void SpecificWorker::draw_lidar(auto &filtered_points,
  ...
  auto wall_res_right = closest_lidar_index_to_given_angle(
      filtered_points, params.LIDAR_RIGHT_SIDE_SECTION);
  auto wall_res_left = closest_lidar_index_to_given_angle(
      filtered_points, params.LIDAR_LEFT_SIDE_SECTION);
  ...
  auto min_obj = (right_point.distance2d < left_point.distance2d) ? right_point
                                                                  : left_point;
  // std::cout << right_point.r << " " << left_point.r << " " <<
  // wall_res_right.value() << " " << wall_res_left.value() << " Size: " <<
  // filtered_points.size() << std::endl;
  auto item =
      scene->addRect(-100, -100, 200, 200, QColor(QColorConstants::Svg::orange),
  ...
  auto item_line =
      scene->addLine(QLineF(QPointF(0.f, 0.f), QPointF(min_obj.x, min_obj.y)),
  ...
  auto res_right = closest_lidar_index_to_given_angle(
      filtered_points, params.LIDAR_FRONT_SECTION);
  auto res_left = closest_lidar_index_to_given_angle(
      filtered_points, -params.LIDAR_FRONT_SECTION);
  ...
  QLineF line_left{QPointF(0.f, 0.f),
                   robot_draw->mapToScene(left_line_length * sin(angle1),
                                          left_line_length * cos(angle1))};
  QLineF line_right{QPointF(0.f, 0.f),
                    robot_draw->mapToScene(right_line_length * sin(angle2),
                                           right_line_length * cos(angle2))};
  ...
  // draw center point
  auto center = scene->addRect(room_center.x(), room_center.y(), 200, 200,
                               QColor(QColorConstants::Svg::red),
