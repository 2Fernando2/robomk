# Full_Apartment
**Full_Apartment** is the central navigation and cognitive component for the *Robotics 2025* project. 
It implements a hybrid control architecture designed to solve autonomous indoor exploration, global localization and topological navigation.

**Actividad4** implements both **Full_Apartment** and **mnist_detector** components. Previous activities implements initial stages of this component:
* **Activity1**: **Chocachoca** - Reactive Coverage Strategy.
* **Activity2**: **Localizer** - Global Feature-Based Localization.
* **Activity3**: **Multi_Room** - Multi-Room Navigation.
* **Activity4**: **Full_Apartment** - Multi-Door Navigation.

The component is built upon the **RoboComp** framework and executes a State Machine that orchestrates the following capabilities:
* **Reactive Area Coverage:** Combines deterministic wall-following with stochastic spiral exploration to cover free space in unknown environments.
* **Global Localization:** Solves the "lost robot problem" by matching real-time LiDAR features (lines/corners) against a known map using **RANSAC** and the **Hungarian Algorithm** for robust data association.
* **Multi-Room Navigation:** Detects geometric apertures (doors) and manages topological transitions between rooms using a coordinate frame hand-off system.
* **Semantic Identification:** Integrates a Deep Neural Network (DNN) via the `mnist_detector` to recognize handwritten digits on walls, allowing for unique semantic identification of rooms and loop closure in complex apartment layouts.

The system relies on a **Componed-Based Software Engineering (CBSE)** approach, communicanting via ZeroC Ice with hardware abstraction layers (Lidar3D, Camera360, Omnirobot).

## Dependencies
This component relies on the following libraries and tools (please, ensure they are all installed):

* **RoboComp Framework**: core middleware for communication and component generation.
* **ZeroC Ice**: Middleware for RPC communication.
* **OpenCV (4.x)**: Required for image processing, finding contuours and visualizing the local map.
* **Eigen3**: Used for matrix operations, geometric transformations and the Hungarian Algorithm implementation.
* **cppitertools**: Used for efficient iteration over data structures.
* **Qt6**: Used for the State Machine, GUI and signals/slots mechanism.
* **QCustomPlot**: Used for real-time error telemetry plotting.

## Configuration parameters
This component requires connection to several hardware interfaces (proxies):

* **CommonBehavior Proxy**: Interface for generic robot state.
* **Lidar3D Proxy**: Source of point cloud data for localization and door detection.
* **OmniRobot Proxy**: Control interface for the holonomic platform (base).

Other optional components:
* **Camera360RGB Proxy**: For visual landmark detection.
* **MNIST Proxy (mnist_detector)**: For semantic digit recognition.
