...
{
    /**
     * @brief Class responsible for reconstructing room geometry from Lidar data.
     * Uses line detection to find walls and computes corners (intersections) to define the room shape.
     */
    class Room_Detector
    {
        public:
             /** @brief Computes corners and lines from a set of 2D Eigen points.
              * @param line Input points.
              * @param scene Optional graphics scene for debug drawing.
              * @return Tuple containing {Detected Corners, Detected lines}.
              */
             std::tuple<Corners, Lines> compute_corners(const std::vector<Eigen::Vector2d> &line, QGraphicsScene *scene= nullptr);

             /** @brief Overload for 3D Eigen points (projects to 2D). */
             std::tuple<Corners, Lines> compute_corners(const std::vector<Eigen::Vector3d> &line, QGraphicsScene *scene= nullptr);

             /** @brief Overload for RoboComp Lidar points. */
            ...
            // local data - conversion helpers
             ...
             /** @brief Estimates room center based on detected wall lines.
             * Solves for the center of the best fitting rectangle defined by the lines.
             *   ax + by + c = 0 in Eigen::Vector3d (a,b,c). Lines can be unnormalized.
             * @param lines Detected line segments.
             * @return Optional center coordinates. Returns std::nullpt if not enough info (e.g., missing one direction).
             */
             std::optional<Eigen::Vector2d> estimate_center_from_walls(const Lines &lines) const;

             /** @brief Overload that uses internally stored lines (if any). */
             std::optional<Eigen::Vector2d> estimate_center_from_walls() const;

             /**
              * @brief Selects the best 4 corners forming a rectangle from a large set of candidates.
              * @param corners Candidate corners.
              * @return The 4 selected corners.
              */
