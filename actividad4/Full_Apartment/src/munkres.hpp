...
{
/**
* @brief Internal state structure for the Munkres algorithm.
* Manages the cost matrix, masks and step transitions.
* @tparam T Type of the cost values (e.g., double, float, int).
*/
...
/**
 * @brief Solves the Assignment Problem using the Munkres (Hungarian) Algorithm.
 * Finds the combination of assignments that minimizes the total cost in a bipartite graph.
 ...
 * @see example.cpp
 * @return A list of pairs representing the optimal assignment indices.
 * Only valid assignments are returned.
 */
template<typename T>
...
template<typename T>
std::vector<std::pair<unsigned, unsigned>> inline hungarian( const Eigen::MatrixX<T>& cost_matrix) noexcept
 {
   const unsigned n_lhs_verts = cost_matrix.rows();
   const unsigned n_rhs_verts = cost_matrix.cols();
   detail::MunkresData<T> m_dat{n_lhs_verts, n_rhs_verts, [cost_matrix](auto l, auto r){ return cost_matrix(l, r); }};
   return m_dat.solve();
}
